...
for lines in data.splitlines():
    #The first part of the line is the policy
    policy = lines.split(':')[0] # the policy
    charange = policy.split(' ')[0]
    lowbound = int(charange.split("-")[0])
    highbound = int(charange.split("-")[1])
    character = policy.split(' ')[1] # Specfied character

    #The second part of the line is the password
    password = lines.split(':')[1] # the password
    if not(int(lowbound) <= password.count(character) <= int(highbound)):
        total += 1
        print(password)
print(total)
